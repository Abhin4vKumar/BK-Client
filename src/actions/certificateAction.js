...
export const getMyCertificate = (id) => async (dispatch)=>{
  try{
    ...
    fetch(`https://bhartiyakanoon.onrender.com/api/v1/myCertificates?token=${encodeURIComponent(token)}` , { mode : "cors" , credentials : "include" } ).then(response => response.json()).then(data => {
...
export const createCertificate = (productData) => async (dispatch) => {
  try {
    ...
    const token = Cookies.get('token');
    fetch(`https://bhartiyakanoon.onrender.com/api/v1/certificate/new?token=${encodeURIComponent(token)}` , {
      method : "POST",
      mode : "cors",
      headers : {
